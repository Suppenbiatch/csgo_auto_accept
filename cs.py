...
try:
    ...
    class ConfigItems:
        ...
        open_live_tab: str = config.get('HotKeys', 'Live Tab Key')
        ...
        max_queue_position: int = config.getint('csgostats.gg', 'Auto-Retrying for queue position below')
        auto_retry_interval: int = config.getint('csgostats.gg', 'Auto-Retrying-Interval')
        discord_url: str = config.get('csgostats.gg', 'Discord Webhook URL')
        player_webhook: str = config.get('csgostats.gg', 'Player Info Webhook')
